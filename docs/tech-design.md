...
- `serial.full.log`：连续全量日志，不截断。
- `serial.current.log`：当前 boot cycle。
...
- `serial` 实时日志通过 `serial_poll_logs` 或读取 `serial.current.log` 实现。
...
- 监听 `serial.current.log`：CLI `serial` 实时显示增量。
