...
//! - `serial.current.log` — new log data → CLI real-time display
...
pub enum WatchKind {
    ...
    ConfigChanged,
    /// `serial.current.log` was appended to → new log data available.
    LogAppended,
...
impl InotifyWatcher {
    ...
    pub fn new(project_dir: &Path, dut_dir: &str) -> Result<Self, String> {
        ...
        // Watch log directory for serial.current.log changes
        if log_dir.exists() {
            inotify
                .watches()
                .add(
    ...
    pub fn wait(&mut self) -> Result<WatchEvent, String> {
        loop {
            ...
            for ev in events {
                ...
                } else if name == "serial.current.log" {
