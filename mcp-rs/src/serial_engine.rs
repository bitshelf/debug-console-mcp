...
impl SerialEngine {
    ...
    pub async fn start(&mut self) -> Result<(), String> {
        ...
        if let Some(conflicting_pid) = lock_manager::check_project_singleton(&project_dir, &dut_dir)
        {
            ...
            if lock_manager::process_alive(conflicting_pid) {
                tracing::warn!(
                    "Old MCP process {conflicting_pid} did not exit — sending SIGKILL"
                );
            ...
            .await
        {
            Ok(data) if !data.is_empty() => {
                ...
                if has_real_data {
                    ...
                    // Reset hang timers — probe just confirmed the board is alive.
                    // Without this, last_data_time stays stale (from startup or a
                    // prior session) and the watchdog fires DUT-off prematurely.
                    ...
                    .await
                {
                    ...
                    // 2nd read IO error (connection reset) — board is unreachable.
                    // Don't report "active" when the TCP connection just dropped.
                    self.state.transition(TargetState::Disconnected);
                    tracing::warn!("Probe: 2nd read error → disconnected");
                }
            }
            Ok(_) => {
                // Initial read returned nothing. Send a blank line to
                // distinguish "board idle at prompt" from "board unplugged".
                self.console.sendline("");
                self.console.drain_writes().await;
                tokio::time::sleep(Duration::from_millis(500)).await;
                if let Ok(d2) = self
                    ...
                    .await
                {
                    if !d2.is_empty() {
                        ...
                        // No response to probe — board is likely unplugged/hung.
                        self.state.transition(TargetState::DutOff);
                        tracing::warn!("Probe: no data, no probe response → DUT-off");
                    }
                } else {
                    // 2nd read IO error after blank-line probe → board unreachable.
                    self.state.transition(TargetState::Disconnected);
    ...
    /// Clear WebSocket broadcast channel.
