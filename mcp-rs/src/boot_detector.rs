...
// ── 预编译 Regex ──
static SPL_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"U-Boot\s+SPL").unwrap());
static TPL_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"TL[123]\s").unwrap());
...
static UBOOT_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"(?:U-Boot\s+20\d{2}|^=>\s)").unwrap());
static AUTOBOOT_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"Hit\s+(?:any\s+)?key\s+to\s+stop\s+autoboot").unwrap());
...
static ANDROID_SHELL_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"console:/\s*[#\$]").unwrap());
...
static UNABLE_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"Unable\s+to\s+handle\s+kernel").unwrap());
static BUG_HANDLE_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"BUG:\s+unable\s+to\s+handle").unwrap());
static SEGFAULT_RE: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"Segmentation\s+fault").unwrap());
...
impl std::fmt::Display for StageLearnerError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Io { path, source } => {
                write!(f, "cannot read reference log '{}': {source}", path.display())
            }
...
pub struct BootStageDetector {
    ...
    /// Custom crash substrings from `.target.toml` `[monitor] crash_patterns`.
    custom_crash_patterns: Vec<String>,
}
...
impl BootStageDetector {
    pub fn new() -> Self {
        Self {
            ...
            custom_crash_patterns: Vec::new(),
        }
    }

    /// Set custom crash patterns from `.target.toml` `[monitor] crash_patterns`.
    /// Each entry is a substring (not regex). The built-in regex patterns
    /// (`PANIC_RE`, `BUG_RE`, etc.) are always checked first.
    pub fn set_crash_patterns(&mut self, patterns: Vec<String>) {
        self.custom_crash_patterns = patterns;
    }
    ...
    pub fn load_reference(
        ...
        let mut learner = if stage_threshold > 0.0 || crash_threshold > 0.0 {
            let st = if stage_threshold > 0.0 { stage_threshold } else { 0.45 };
            let ct = if crash_threshold > 0.0 { crash_threshold } else { 0.50 };
    ...
    fn check_crash(&mut self, line: &[u8], line_str: &str) -> Option<BootEvent> {
        ...
        // 2. Custom substring patterns from .target.toml [monitor] crash_patterns.
        for pat in &self.custom_crash_patterns {
