...
/// read-loop cycle (~100ms worst-case with network jitter).
...
async fn serial_ws_handler(mut socket: WebSocket, state: SharedServer) {
    ...
    // Get write sender and flush stale serial buffer BEFORE registering
    // the broadcast channel. Otherwise partial/leftover data from a previous
    // MCP command may reach the WebSocket client mid-line, causing the first
    // prompt to appear at a non-zero column.
    let write_tx = {
        let eng = engine.lock().await;
        eng.get_write_sender()
    };

    // Trigger a clean prompt and let the read loop consume any stale output
    // before the client sees it.
    let _ = write_tx.send(b"\n".to_vec()).await;
    tokio::time::sleep(WS_FLUSH_DRAIN).await;

    // Now register the broadcast channel — data from here on is clean.
    {
        let mut eng = engine.lock().await;
