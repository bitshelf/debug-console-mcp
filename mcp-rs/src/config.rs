...
impl Config {
    ...
    pub fn power_off_time_ms(&self) -> u64 {
        self.get("POWER_OFF_TIME_MS").parse().unwrap_or(3000).max(3000)
    }
    #[allow(dead_code)]
    ...
    pub fn learner_stage_threshold(&self) -> f64 {
        let default: f64 = option_env!("LEARN_STAGE_THRESHOLD")
            ...
            .unwrap_or(0.45);
        self.get("LEARNER_STAGE_THRESHOLD").parse().unwrap_or(default)
    }
    ...
    pub fn learner_crash_threshold(&self) -> f64 {
        let default: f64 = option_env!("LEARN_CRASH_THRESHOLD")
            .and_then(|s| s.parse().ok())
            .unwrap_or(0.50);
        self.get("LEARNER_CRASH_THRESHOLD").parse().unwrap_or(default)
    }
...
impl DutConfig {
    ...
    pub fn to_config_map(&self, global: &HashMap<String, String>) -> HashMap<String, String> {
        ...
        if self.power_off_time_ms > 0 {
            cfg.insert("POWER_OFF_TIME_MS".into(), self.power_off_time_ms.to_string());
        }
        ...
        if self.learner_stage_threshold > 0.0 {
            cfg.insert("LEARNER_STAGE_THRESHOLD".into(), self.learner_stage_threshold.to_string());
        }
        if self.learner_crash_threshold > 0.0 {
            cfg.insert("LEARNER_CRASH_THRESHOLD".into(), self.learner_crash_threshold.to_string());
        }
...
struct TargetToml {
    // [[dev_hosts]] array of tables — the only supported format
    #[serde(default)]
