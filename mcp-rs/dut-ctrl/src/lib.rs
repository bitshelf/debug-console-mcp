...
pub struct Ch340RelayControl {
    ...
    min_reset_ms: u64,
    /// Minimum power-off duration in ms. Power cycle off-time is clamped
    /// to at least this value. Default: 3000 (3s).
...
impl Ch340RelayControl {
    ...
    /// Override the minimum power-off duration (ms). Clamped to >= 3000.
    pub fn with_min_power_off_ms(mut self, ms: u64) -> Self {
        self.min_power_off_ms = ms.max(3000);
        self
    }

    /// Power cycle the DUT: power off (>= 3s) → power on.
    pub async fn power_cycle(&mut self) -> Result<(), PowerError> {
        if self.channels.power == 0 {
            return Err(PowerError::NotConfigured);
        }
        tracing::info!("Power cycle: OFF for {}ms", self.min_power_off_ms);
        self.press(Button::Power).await?;
        tokio::time::sleep(Duration::from_millis(self.min_power_off_ms)).await;
        self.release(Button::Power).await?;
        Ok(())
    }
    ...
    async fn send_command(
        ...
        for attempt in 0..2 {
            ...
            match tokio::time::timeout(Duration::from_millis(500), stream.read(&mut buf)).await {
                Ok(Ok(n)) if n >= 4 => {
                    if let Some(pos) = buf[..n].iter().position(|&b| b == Self::HEADER) {
                        let echo = &buf[pos..];
                        if echo.len() >= 4
                            && echo[1] == channel
                            && echo[2] == opcode
                        {
                            return Ok(echo.to_vec());
                        }
                    }
                    // Echo didn't match — still in spec (relay may not echo).
                    // Return empty to avoid breaking existing flow.
                    return Ok(Vec::new());
                }
                _ => return Ok(Vec::new()), // timeout or no data — acceptable
            }
        }
