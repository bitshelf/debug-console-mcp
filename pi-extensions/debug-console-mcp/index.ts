/**
 * debug-console MCP Client — pi.dev bridge for the Rust debug-console-mcp server
 *
 * ═══════════════════════════════════════════════════════════════════
 *  WHY THIS EXISTS
 * ═══════════════════════════════════════════════════════════════════
 ...
 *  MCP server (`debug-console-mcp` binary, stdio transport). This
 *  extension is the missing client side:
 *
 *    1. Finds the debug-console-mcp binary (env / PATH / project bin /
 ...
 * ═══════════════════════════════════════════════════════════════════
 *   1. $DEBUG_CONSOLE_MCP_BIN          (explicit override)
 *   2. project .mcp.json mcpServers["debug-console"].command
 *   3. PATH lookup `debug-console-mcp`
 *   4. ~/.local/bin/debug-console-mcp  (standard install location)
 *   5. project bin/debug-console-mcp
 *   6. package-local bin/debug-console-mcp (plugin tarball layout)
 *
...
const SERVER_NAME = "debug-console";
...
/** Read the `debug-console` command from a project .mcp.json, if present. */
...
function findBinary(projectDir: string | null): string | undefined {
  const env = process.env.DEBUG_CONSOLE_MCP_BIN;
  ...
  const inPath = findOnPath("debug-console-mcp");
  if (inPath) return inPath;

  const homeLocal = path.join(os.homedir(), ".local", "bin", "debug-console-mcp");
  if (fileExistsAndExecutable(homeLocal)) return homeLocal;

  if (projectDir) {
    const projBin = path.join(projectDir, "bin", "debug-console-mcp");
    if (fileExistsAndExecutable(projBin)) return projBin;
  }

  // Package-local bin/ (plugin tarball layout: <pkg>/bin/debug-console-mcp).
  const pkgBin = path.join(__dirname, "..", "..", "bin", "debug-console-mcp");
...
class McpStdioClient {
  ...
  async start(): Promise<McpToolInfo[]> {
    ...
    this.child.stderr.on("data", (d: Buffer) => {
      for (const line of d.toString().split("\n")) {
        if (line.trim()) this.onLog(`[debug-console-mcp] ${line.trim()}`);
      }
    });

    this.child.on("exit", (code, signal) => {
      const err = new Error(
        `debug-console-mcp server exited (code=${code}, signal=${signal ?? "none"})`,
      );
    ...
    const init = await this.request(
      "initialize",
      {
        ...
        capabilities: {},
        clientInfo: { name: "pi", version: "debug-console-mcp-bridge" },
      },
      INIT_TIMEOUT_MS,
    );
  ...
  private send(msg: Record<string, unknown>): void {
    if (!this.child?.stdin.writable) throw new Error("debug-console-mcp server not running");
...
export default function (pi: ExtensionAPI) {
  ...
  async function startServer(ctx: ExtensionContext): Promise<void> {
    ...
    if (!binary) {
      ctx.ui.notify(
        "debug-console-mcp binary not found. Install to ~/.local/bin or set DEBUG_CONSOLE_MCP_BIN",
        "warning",
      );
    ...
    try {
      ...
      ctx.ui.notify(`debug-console MCP start failed: ${msg}`, "error");
      client.stop();
      return;
    }
    ...
    for (const tool of tools) {
      ...
      const description = tool.description || `debug-console MCP tool: ${name}`;
      ...
      pi.registerTool({
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, toolCtx) {
          ...
          if (!client?.running) {
            throw new Error("debug-console MCP server is not running");
          }
    ...
    ctx.ui.notify(`debug-console MCP: ${count} serial tools ready`, "info");
  }
  ...
  pi.registerTool({
    ...
    description:
      "Report the debug-console MCP client status: binary path, project dir, " +
        ...
        content: [
          {
            ...
            text: [
              ...
              `install_hint: ${
                binary
                  ? "ok"
                  : "install via: curl -L https://github.com/bitshelf/debug-console-mcp/releases/latest/download/debug-console-mcp-$(uname -m) -o ~/.local/bin/debug-console-mcp && chmod +x ~/.local/bin/debug-console-mcp"
              }`,
            ].join("\n"),
          },
        ],
