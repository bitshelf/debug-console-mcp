# pi.dev — debug-console MCP Client Bridge
...
is the missing client side: it spawns the `debug-console-mcp` binary (stdio
...
pi (agent)  ──►  pi.registerTool("serial_get_state") ──┐
                                                       │ stdio JSON-RPC
                       pi-extensions/debug-console-mcp │ (newline-delimited)
                                                       ▼
                                       debug-console-mcp binary (Rust MCP server)
                                                       │
...
```bash
pi install .
# or, from anywhere: pi install /path/to/debug-console-mcp
```

The extension is loaded from the package manifest
(`pi.extensions: ["./pi-extensions/debug-console-mcp"]`). Restart pi or run
...
2. project `.mcp.json` → `mcpServers["debug-console"].command` (with `${VAR}` expansion)
