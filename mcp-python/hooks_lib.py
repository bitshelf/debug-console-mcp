...
def find_project_dir() -> Optional[str]:
    """Walk up from CWD to find .target.conf. Returns project dir or None."""
    if env := os.environ.get("TARGET_CONF"):
        if Path(env).exists():
            return str(Path(env).resolve().parent)
    d = Path.cwd()
    while True:
    ...
    for entry in base.iterdir():
        ...
        try:
            for line in target_conf.read_text().splitlines():
                line = line.strip()
                if line.startswith("RK_DEV_HOST_IP="):
                    conf_host = line.split("=", 1)[1].strip().strip('"').strip("'")
                elif line.startswith("RK_SERIAL_PORT="):
...
def check_mcp_alive(project_dir: str) -> bool:
    ...
    StateManager writes .dut-serial/mcp.pid at startup.
    Returns True if the process exists, False otherwise.
    """
    pid_file = Path(project_dir) / ".dut-serial" / "mcp.pid"
    if not pid_file.exists():
        return False
    try:
        pid = int(pid_file.read_text().strip())
